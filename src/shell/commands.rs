...
pub fn cmd_help() {
    ...
    print("  vma        - Dump the Virtual Memory Area table\n", ColorCode::new(Color::White, Color::Black));
...
/// Display the Virtual Memory Area table.
pub fn cmd_vma() {
    let cyan  = ColorCode::new(Color::LightCyan, Color::Black);
    let white = ColorCode::new(Color::White, Color::Black);
    let yel   = ColorCode::new(Color::Yellow, Color::Black);

    print("=== Virtual Memory Areas (", cyan);
    print_dec(crate::paging::vma::count());
    print(" regions) ===\n", cyan);

    print("  Start      End        Size     Kind      Flags\n", white);
    print("  ---------  ---------  -------  --------  -----\n", white);

    crate::paging::vma::for_each(|v| {
        print("  ", white);
        print_hex(v.start as usize);
        print("  ", white);
        print_hex(v.end as usize);
        print("  ", white);
        // Print size in KB
        let size_kb = v.size() as usize / 1024;
        if size_kb >= 1024 {
            print_dec(size_kb / 1024);
            print(" MB", white);
            // Pad
            let digits = count_digits(size_kb / 1024);
            for _ in 0..(4usize.saturating_sub(digits + 3)) { print(" ", white); }
        } else {
            print_dec(size_kb);
            print(" KB", white);
            let digits = count_digits(size_kb);
            for _ in 0..(4usize.saturating_sub(digits + 3)) { print(" ", white); }
        }
        print("  ", white);
        // Kind name (padded to 9 chars)
        let name = v.kind_name();
        print(name, yel);
        let name_len = name.len();
        for _ in 0..(9usize.saturating_sub(name_len)) { print(" ", white); }
        print("  ", white);
        // Flags
        let fstr = v.flags_str();
        for &b in fstr.iter() {
            print_char(b as char, if b == b'-' { white } else { ColorCode::new(Color::LightGreen, Color::Black) });
        }
        print("\n", white);
    });
}

fn count_digits(val: usize) -> usize {
    if val == 0 { return 1; }
    let mut n = val;
    let mut d = 0;
    while n > 0 { n /= 10; d += 1; }
