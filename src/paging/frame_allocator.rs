...
/// Physical address of the VGA text-mode framebuffer (0xB8000).
/// This is a 4 KB region that must never be handed out by the allocator.
const VGA_BUFFER_ADDR: usize = 0x000B_8000;

/// End of the legacy BIOS / ISA hole region that must stay reserved.
/// Covers real-mode IVT (0x0), BDA (0x400), EBDA, VGA ROM, BIOS ROM, etc.
/// Everything from physical 0 up to and including 0xFFFFF (the first 1 MB).
const BIOS_REGION_END: usize = 0x0010_0000;
...
impl FrameAllocator {
    ...
    pub fn init(&mut self, total_memory: usize, kernel_end: usize) {
        ...
        // Step 3 — defensively reserve hardware regions that must never be
        // allocated, even if they happen to sit above kernel_end.
        self.reserve_hardware_regions();
    }

    /// Mark well-known hardware regions as permanently used.
    ///
    /// This is a safety net: the first 1 MB (BIOS/VGA/ROMs) is almost
    /// certainly already below `_kernel_end` and therefore already marked
    /// used, but being explicit protects against linker-script changes and
    /// documents the invariant.
    fn reserve_hardware_regions(&mut self) {
        // Reserve the entire first 1 MB (BIOS, IVT, BDA, VGA, ROMs).
        let frames_in_first_mb = BIOS_REGION_END / PAGE_SIZE; // 256
        for idx in 0..frames_in_first_mb {
            let addr = (idx * PAGE_SIZE) as u32;
            self.mark_used(addr);
        }

        // Explicitly reserve the VGA text framebuffer frame (0xB8000).
        // Already covered by the loop above, but this makes the intent
        // crystal clear for anyone reading the code.
        self.mark_used(VGA_BUFFER_ADDR as u32);
    }
